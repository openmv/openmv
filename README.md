...
OpenMV also provides a Unix port that allows running the image processing library on desktop systems (Linux, Mac, Windows with WSL) without hardware. This is useful for algorithm development, testing, and CI/CD integration. See [Unix Port Documentation](docs/unix-port.md) for details.
