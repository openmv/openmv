...
#define AI_RELOC_RT_VERSION_MAJOR 8   /* for ATON env. */
 ...
 * 	                  b15:      1b - SECURE conf. '1b': secure, '0b': non-secure
...
struct ai_reloc_rt_ctx {
  ...
  struct ai_reloc_callback* cbs;        /* callback functions */
...
typedef struct _ai_rel_mem_pool_desc {
...
/* -----------------------------------------------------------------------------
 * Entry point for LL ATONN runtime
 * -----------------------------------------------------------------------------
 */
...
#if !defined(WEIGHTS_SZ)
#define WEIGHTS_SZ (3333)
#endif
...
#define SECURE_CONF   1
#else
#define SECURE_CONF   0
#endif
#endif
...
#define VARIANT  ( AI_RELOC_RT_SET_TOOLS(ARM_TOOLS_CONF) | AI_RELOC_RT_SET_ABI(_EABI) |\
                   AI_RELOC_RT_SET_FPU(_FPU) | ARM_MCU_CONF | AI_RELOC_RT_SET_SECURE(SECURE_CONF))
#endif
...
#define AI_RELOC_NETWORK()\
    struct ai_reloc_rt_ctx __attribute__((used, section (".network_rt_ctx"), )) _network_rt_ctx = { 0, 0, 0, 0, NULL, \
         C_NAME, ACTS_SZ, PARAMS_SZ, RUNTIME_DESC, RUNTIME_VERSION, RUNTIME_VERSION_DEV, PARAMS_BIN_SZ, PARAMS_BIN_CRC32}; \
    const struct ai_reloc_network_entries __attribute__((used, section (".network_entries"), visibility("default"))) _network_entries = { \
    ...
    const uint32_t __attribute__((used, section (".network_flags"), visibility("default"))) _network_flags = AI_RELOC_RT_SET_FLAGS(VARIANT);\
