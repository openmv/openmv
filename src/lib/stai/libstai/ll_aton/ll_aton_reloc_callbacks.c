...
void __assert_func( const char *filename, int line, const char *assert_func, const char *expr)
{
  ...
  const struct ai_reloc_callback *cbs = _network_rt_ctx.cbs; 
...
int mcu_cache_invalidate_range(uint32_t start_addr, uint32_t end_addr)
{
  ...
  const struct ai_reloc_callback *cbs = _network_rt_ctx.cbs; 
  int res = -1;
  if (cbs && cbs->mcu_cache_invalidate_range) {
    __asm volatile( "MOV %0, R9\n\t" :"=r"(_saved_r9)); 
    res = cbs->mcu_cache_invalidate_range(start_addr, end_addr);
    __asm volatile( "MOV R9, %0\n\t" ::"r"(_saved_r9));
  };
  return res;  
}
...
int mcu_cache_clean_range(uint32_t start_addr, uint32_t end_addr)
{
  ...
  int res = -1;
  if (cbs && cbs->mcu_cache_clean_range) {
    __asm volatile( "MOV %0, R9\n\t" :"=r"(_saved_r9)); 
    res = cbs->mcu_cache_clean_range(start_addr, end_addr);
    __asm volatile( "MOV R9, %0\n\t" ::"r"(_saved_r9));
  };
  return res;  
}
...
void npu_cache_clean_invalidate_range(uint32_t start_addr, uint32_t end_addr)
{
  register uint32_t _saved_r9; 
  register const struct ai_reloc_callback *cbs = _network_rt_ctx.cbs; 
  if (cbs && cbs->npu_cache_clean_invalidate_range) {
    __asm volatile( "MOV %0, R9\n\t" :"=r"(_saved_r9)); 
    cbs->npu_cache_clean_invalidate_range(start_addr, end_addr);
    __asm volatile( "MOV R9, %0\n\t" ::"r"(_saved_r9));
  };
}
...
void npu_cache_clean_range(uint32_t start_addr, uint32_t end_addr)
{
  register uint32_t _saved_r9; 
  register const struct ai_reloc_callback *cbs = _network_rt_ctx.cbs; 
  if (cbs && cbs->npu_cache_clean_range) {
    __asm volatile( "MOV %0, R9\n\t" :"=r"(_saved_r9)); 
    cbs->npu_cache_clean_range(start_addr, end_addr);
    __asm volatile( "MOV R9, %0\n\t" ::"r"(_saved_r9));
  };
}
                               ...
                               unsigned stride_h, unsigned stride_w, int dma_in, int dma_out)
{
  ...
  if (cbs && cbs->ll_aton_lib_dma_imagetorow) {
    __asm volatile( "MOV %0, R9\n\t" :"=r"(_saved_r9)); 
    res = cbs->ll_aton_lib_dma_imagetorow(inputs, ninputs, output, blocksize_h, blocksize_w, stride_h,
                                          stride_w, dma_in, dma_out);
    __asm volatile( "MOV R9, %0\n\t" ::"r"(_saved_r9));
  };
  return res; 
}
                                 ...
                                 int dma_in, int dma_out)
{
  ...
  if (cbs && cbs->ll_aton_lib_dma_spacetodepth) {
    __asm volatile( "MOV %0, R9\n\t" :"=r"(_saved_r9)); 
    res = cbs->ll_aton_lib_dma_spacetodepth(inputs, ninputs, output, blocksize_h, blocksize_w,
                                            dma_in, dma_out);
    __asm volatile( "MOV R9, %0\n\t" ::"r"(_saved_r9));
  };
  return res; 
                               ...
                               unsigned stride_h, unsigned stride_w, int dma_in, int dma_out)
{
  ...
  if (cbs && cbs->ll_aton_lib_dma_rowtoimage) {
    __asm volatile( "MOV %0, R9\n\t" :"=r"(_saved_r9)); 
    res = cbs->ll_aton_lib_dma_rowtoimage(inputs, ninputs, output, blocksize_h, blocksize_w, stride_h,
                                          stride_w, dma_in, dma_out);
    __asm volatile( "MOV R9, %0\n\t" ::"r"(_saved_r9));
  };
  return res; 
                                 ...
                                 int dma_in, int dma_out)
{
  ...
  if (cbs && cbs->ll_aton_lib_dma_depthtospace) {
    __asm volatile( "MOV %0, R9\n\t" :"=r"(_saved_r9)); 
    res = cbs->ll_aton_lib_dma_depthtospace(inputs, ninputs, output, blocksize_h, blocksize_w,
                                            dma_in, dma_out);
    __asm volatile( "MOV R9, %0\n\t" ::"r"(_saved_r9));
  };
  return res; 
                              ...
                              const uint8_t *target_pos, const uint8_t *perm_to_use, int dma_in, int dma_out)
{
  ...
  if (cbs && cbs->ll_aton_lib_dma_transpose) {
    __asm volatile( "MOV %0, R9\n\t" :"=r"(_saved_r9)); 
    res = cbs->ll_aton_lib_dma_transpose(input, input_axes_offsets, output, output_axes_offsets,
                                         target_pos, perm_to_use, dma_in, dma_out);
    __asm volatile( "MOV R9, %0\n\t" ::"r"(_saved_r9));
  };
  return res; 
}
                                            ...
                                            int dma_in, int dma_out)
{
  ...
  if (cbs && cbs->ll_aton_lib_dma_outputs_slice_splitlike) {
    __asm volatile( "MOV %0, R9\n\t" :"=r"(_saved_r9)); 
    res = cbs->ll_aton_lib_dma_outputs_slice_splitlike(input, output, tot_out_size, width_in_bytes,
                                         fheight, line_offset, n_bits, dma_in, dma_out);
    __asm volatile( "MOV R9, %0\n\t" ::"r"(_saved_r9));
  };
  return res; 
}
                                               ...
                                               unsigned int leading_dims, int dma_in, int dma_out)
{
  ...
  if (cbs && cbs->ll_aton_lib_dma_outputs_channel_split_aton) {
    __asm volatile( "MOV %0, R9\n\t" :"=r"(_saved_r9)); 
    res = cbs->ll_aton_lib_dma_outputs_channel_split_aton(input, outputs, nr_of_outputs, leading_dims,
                                                          dma_in, dma_out);
    __asm volatile( "MOV R9, %0\n\t" ::"r"(_saved_r9));
  };
  return res; 
