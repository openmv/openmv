...
#else
#define AI_RELOC_LOG(...) do { } while ( 0 )
#endif
...
#define AI_RELOC_IN_RAM(_val) \
    (((_val) & 0xF0000000) == AI_RELOC_RAM_BASE)

#define AI_RELOC_IN_FLASH(_val) \
    (((_val) & 0xF0000000) == AI_RELOC_FLASH_BASE)

#define AI_RELOC_IN_PARAM_0(_val) \
    (((_val) & 0xF0000000) == AI_RELOC_PARAM_0_BASE)

#define AI_RELOC_IN_PARAM_1(_val) \
    (((_val) & 0xF0000000) == AI_RELOC_PARAM_1_BASE)
...
#define RELOC_MCU_CLEAN_INVALIDATE(_addr, _size)  SCB_CleanInvalidateDCache_by_Addr((void *)(_addr), (_size));
#else
#define RELOC_MCU_CLEAN_INVALIDATE(_addr, _size)  do { } while ( 0 )
#endif

#if defined(STM32N6)
#define RELOC_NPU_INVALIDATE()  npu_cache_invalidate()
#else
#define RELOC_NPU_INVALIDATE()  do { } while ( 0 )
#endif
...
static int _ai_reloc_rt_checking(const struct ai_reloc_bin_hdr *bin)
{
  ...
  if (cpuid != AI_RELOC_RT_GET_CPUID(flags)) {
    AI_RELOC_LOG("AI RELOC ERROR: CPUID is invalid 0x%03X (expected 0x%03X)\r\n",
                 (int)cpuid, (int)AI_RELOC_RT_GET_CPUID(flags));
    return AI_RELOC_RT_ERR_INVALID_BIN;
  }
  ...
  if (rt_vers_ != (rt_ctx->rt_version & 0xFFFFFF00UL)) {
    AI_RELOC_LOG("AI RELOC ERROR: RT version is invalid 0x%03X (%s) (expected 0x%03X)\r\n",
                 (int)rt_vers_, rt_ctx->rt_version_desc, (int)rt_ctx->rt_version);
    return AI_RELOC_RT_ERR_INVALID_BIN;
  }
...
static int _ai_reloc_prepare_mpools(const uintptr_t file_ptr,
                                    struct id_mpool_mapping *id_map,
                                    uint32_t mode)
{
  ...
  ai_rel_mem_pool_desc* cur_mem_c_desc;

  /* Set/check base param addr - user addr is used in priority */
  if ((id_map->addr_0 == 0) && AI_RELOC_GET_OFFSET(header->sect.params_offset))
  ...
  int cur_index = 0;
  cur_mem_c_desc =  (ai_rel_mem_pool_desc*)AI_RELOC_GET_ADDR(header, params_start);
...
static int _ai_reloc_got_update(const struct ai_reloc_bin_hdr *bin, uintptr_t ram_addr,
                                uintptr_t param_0_addr, uintptr_t param_1_addr)
{
 ...
 */
int _ai_reloc_ram_update(const struct ai_reloc_bin_hdr *bin, uintptr_t ram_addr,
                         uintptr_t param_0_addr, uintptr_t param_1_addr, const uintptr_t obj)
{
 ...
 *
 */
static int _ai_reloc_install(const uintptr_t file_ptr, uintptr_t ram_addr, size_t ram_size,
                             uint32_t ext_ram_addr, size_t ext_ram_size,
                             uint32_t param_addr,
                             uintptr_t* hdl, uint32_t mode)
{
  int res;
...
int ai_rel_get_file_ptr(uintptr_t hdl, uintptr_t *file_ptr)
{
  if (!hdl)
    return AI_RELOC_RT_ERR_INVALID_HDL;
  ...
  if (!bin || (bin->hdr.magic != AI_RELOC_MAGIC) || (!file_ptr) )
    return AI_RELOC_RT_ERR_INVALID_BIN;
...
ai_rel_mem_pool_desc* ai_rel_network_get_mem_pool_desc(const uintptr_t file_ptr, int index)
{
  ...
  cur_mem_c_desc =  (ai_rel_mem_pool_desc*)AI_RELOC_GET_ADDR(bin, off_params);
...
int ai_rel_network_rt_get_info(const uintptr_t file_ptr, ai_rel_network_info* rt)
{
  const struct ai_reloc_bin_hdr *bin = (struct ai_reloc_bin_hdr *)file_ptr;
...
void ai_rel_log_info(const uintptr_t file_ptr)
{
#if defined(APP_DEBUG) && APP_DEBUG == 1
  ...
  ai_rel_network_rt_get_info((uintptr_t)bin, &rt_info);
  ...
  printf("  rt_version    : %d.%d.%d-%d\r\n", (int)(rt_info.rt_version >> 24 & 0xFF), (int)(rt_info.rt_version >> 16 & 0xFF),
         (int)(rt_info.rt_version >> 8 & 0xFF), (int)(rt_info.rt_version_extra));

  printf("\n C-mempool descriptors\n");
  printf(" --------------------------------------------------------------\n");

  ai_rel_mem_pool_desc* mem_c_desc;
  int index = 0;

  while ((mem_c_desc = ai_rel_network_get_mem_pool_desc((uintptr_t)bin, index))) {
    printf(" %d: flags=%lx foff=%ld dst=%lx s=%ld %s\n", index,
           mem_c_desc->flags,
           mem_c_desc->foff,
           mem_c_desc->dst,
           mem_c_desc->size,
           (char *)AI_RELOC_GET_ADDR(bin, (uint32_t)mem_c_desc->name)
          );
    index++;
  }
  ...
  printf("  flags         : v%d.%d secure=%ld compil=%ld feabi=%ld (0x%08X)\r\n",
      AI_RELOC_RT_GET_MAJOR(rt_info.variant),
      AI_RELOC_RT_GET_MINOR(rt_info.variant),
      AI_RELOC_RT_SECURE(rt_info.variant),
      AI_RELOC_RT_GET_COMPILER(rt_info.variant),
      AI_RELOC_RT_GET_FPEABI(rt_info.variant),
      (int)rt_info.variant);
  printf("                  fpu=%ld (is_enabled:%d)\r\n", AI_RELOC_RT_FPU_USED(rt_info.variant), fpu_is_enabled);
  printf("                  cpuid=0x%03lx (0x%03x)\r\n", AI_RELOC_RT_GET_CPUID(rt_info.variant), _GET_PART_NUMBER());
...
int ai_rel_network_install(const uintptr_t file_ptr, uintptr_t ram_addr, uint32_t ram_size,
                           uint32_t ext_ram_addr, size_t ext_ram_size,
                           uint32_t param_addr,
                           uint32_t mode, uintptr_t *inst)
{
  if (!inst || !file_ptr) {
    return AI_RELOC_RT_ERR_INVALID_BIN;
  }

  return _ai_reloc_install(file_ptr, ram_addr, ram_size, ext_ram_addr, ext_ram_size, param_addr, inst, mode);
}

int  ai_rel_network_set_callbacks(uintptr_t inst, const struct ai_reloc_callback *cbs)
{
  if (_ai_rel_check_handler(inst))
    return 0;

  struct ai_reloc_rt_ctx *rt_ctx = (struct ai_reloc_rt_ctx *)inst;
  rt_ctx->cbs = (void *)cbs;
...
{
  if (_ai_rel_check_handler(ARM_PMU_INST_SPEC))
    return NULL;
