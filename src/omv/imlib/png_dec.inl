...
//
static int32_t dec_readMem(PNGFILE *pFile, uint8_t *pBuf, int32_t iLen)
{
    int32_t iBytesRead;
...
static int32_t dec_seekMem(PNGFILE *pFile, int32_t iPosition)
{
...
#ifndef __cplusplus
// C API
int PNGDEC_openRAM(PNG_DEC_IMAGE *pPNG, uint8_t *pData, int iDataSize, PNG_DRAW_CALLBACK *pfnDraw)
{
    pPNG->iError = PNG_SUCCESS;
    pPNG->pfnRead = dec_readMem;
    pPNG->pfnSeek = dec_seekMem;
...
#ifdef __LINUX__
int PNGDEC_openFile(PNG_DEC_IMAGE *pPNG, const char *szFilename, PNG_DRAW_CALLBACK *pfnDraw)
{
    pPNG->iError = PNG_SUCCESS;
...
#endif // __LINUX__
void PNGDEC_close(PNG_DEC_IMAGE *pPNG)
{
    if (pPNG->pfnClose)
        (*pPNG->pfnClose)(pPNG->PNGFile.fHandle);
} /* PNGDEC_close() */

int PNGDEC_getWidth(PNG_DEC_IMAGE *pPNG)
{
    return pPNG->iWidth;
} /* PNGDEC_getsWidth() */

int PNGDEC_getHeight(PNG_DEC_IMAGE *pPNG)
{
    return pPNG->iHeight;
} /* PNGDEC_getHeight() */

int PNGDEC_getLastError(PNG_DEC_IMAGE *pPNG)
{
    return pPNG->iError;
} /* PNGDEC_getLastError() */

uint8_t *PNGDEC_getPalette(PNG_DEC_IMAGE *pPNG)
{
    return pPNG->ucPalette;
} /* PNGDEC_getPalette() */

int PNGDEC_getBufferSize(PNG_DEC_IMAGE *pPNG)
{
    return pPNG->iHeight * pPNG->iPitch;
} /* PNGDEC_getBufferSize() */

uint8_t * PNGDEC_getBuffer(PNG_DEC_IMAGE *pPNG)
{
    return pPNG->pImage;
} /* PNGDEC_getBuffer() */
...
//
PNG_STATIC int PNGParseInfo(PNG_DEC_IMAGE *pPage)
{
...
//
PNG_STATIC int PNGInit(PNG_DEC_IMAGE *pPNG)
{
...
//
PNG_STATIC int DecodePNG(PNG_DEC_IMAGE *pPage, void *pUser, int iOptions)
{
    int err, y, iLen=0;
