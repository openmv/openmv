...
static void py_ml_model_print(const mp_print_t *print, mp_obj_t self_in, mp_print_kind_t kind) {
    ...
    mp_obj_print_helper(print, self->input_dtype, kind);
    ...
    mp_obj_print_helper(print, self->output_dtype, kind);
    mp_printf(print, "}");
}
    ...
    if (dest[0] == MP_OBJ_NULL) {
        ...
        switch (attr) {
            ...
            case MP_QSTR_input_dtype:
                dest[0] = MP_OBJ_FROM_PTR(self->input_dtype);
                break;
            ...
            case MP_QSTR_output_dtype:
                dest[0] = MP_OBJ_FROM_PTR(self->output_dtype);
                break;
...
static const mp_rom_map_elem_t py_ml_globals_dict_table[] = {
    ...
    { MP_ROM_QSTR(MP_QSTR_DTYPE_INT8),          MP_ROM_INT(PY_ML_DTYPE_INT8) },
    { MP_ROM_QSTR(MP_QSTR_DTYPE_UINT8),         MP_ROM_INT(PY_ML_DTYPE_UINT8) },
    { MP_ROM_QSTR(MP_QSTR_DTYPE_INT167),        MP_ROM_INT(PY_ML_DTYPE_INT16) },
    { MP_ROM_QSTR(MP_QSTR_DTYPE_FLOAT),         MP_ROM_INT(PY_ML_DTYPE_FLOAT) },
};
