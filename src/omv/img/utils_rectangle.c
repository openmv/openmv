...
{
    int x0 = ptr0->p.x, y0 = ptr0->p.y, x1 = ptr1->p.x, y1 = ptr1->p.y;
    unsigned int w0 = ptr0->s.w, h0 = ptr0->s.h, w1 = ptr1->s.w, h1 = ptr1->s.h;
...
{
    int leftX = OTHER_MAX(dst->p.x, src->p.x), topY = OTHER_MAX(dst->p.y, src->p.y);
    int rightX = OTHER_MIN(dst->p.x + dst->s.w, src->p.x + src->s.w), bottomY = OTHER_MIN(dst->p.y + dst->s.h, src->p.y + src->s.h);
    dst->p.x = leftX;
    dst->p.y = topY;
...
{
    int leftX = OTHER_MIN(dst->p.x, src->p.x), topY = OTHER_MIN(dst->p.y, src->p.y);
    int rightX = OTHER_MAX(dst->p.x + dst->s.w, src->p.x + src->s.w), bottomY = OTHER_MAX(dst->p.y + dst->s.h, src->p.y + src->s.h);
    dst->p.x = leftX;
    dst->p.y = topY;
    dst->s.w = rightX - leftX;
    dst->s.h = bottomY - topY;
