...
static uint32_t snapshot_in_progress = 0;
...
int sensor_set_windowing(int x, int y, int w, int h)
{
    ...
    MAIN_FB()->bpp = -1; // Skip first frame.
    return 0;
}
...
// This function is called back after each line transfer is complete, with a pointer to the line buffer
// that was used. At this point the DMA transfers the next line to the other half of the line buffer.
void DCMI_DMAConvCpltUser(uint32_t addr)
{
    ...
    uint8_t *dst = (uint8_t*) sensor.buffers[sensor.buf_idx];

    uint16_t *src16 = (uint16_t*) addr;
    uint16_t *dst16 = (uint16_t*) sensor.buffers[sensor.buf_idx];
    ...
    if (offset >= MAIN_FB()->y && offset <= (MAIN_FB()->y + MAIN_FB()->v)) {
        if (!sensor.transpose) {
            switch (sensor.pixformat) {
                case PIXFORMAT_BAYER:
                    dst += (offset - MAIN_FB()->y) * MAIN_FB()->u;
                    src += MAIN_FB()->x;
                ...
                case PIXFORMAT_YUV422:
                case PIXFORMAT_RGB565:
                    dst16 += (offset - MAIN_FB()->y) * MAIN_FB()->u;
                    src16 += MAIN_FB()->x;
...
static void sensor_start_snapshot(sensor_t *sensor, uint32_t addr, uint32_t length, uint32_t h)
{
    ...
    if ((sensor->pixformat == PIXFORMAT_JPEG) && (sensor->chip_id != OV5640_ID)) {
        ...
        // data to. We only use this for JPEG mode for the OV2640.
        HAL_DCMI_Start_DMA(&DCMIHandle, DCMI_MODE_SNAPSHOT, addr, length/4);
    } else {
        // Start a multibuffer transfer (line by line). The DMA hardware will ping-pong
        // transferring data between the uncached line buffers.
        HAL_DCMI_Start_DMA_MB(&DCMIHandle, DCMI_MODE_SNAPSHOT, addr, length/4, h);
    }
    ...
    // Set flag for the next time snapshot is called.
...
int sensor_snapshot(sensor_t *sensor, image_t *image, uint32_t flags)
{
    ...
    if (MAIN_FB()->bpp == -1) {
        // Reset framebuffer index to first buffer.
        sensor->buf_idx    = 0;
    }

    // Compress the framebuffer for the IDE preview, only if it's not the first frame,
    // the framebuffer is enabled and the image sensor does not support JPEG encoding.
    // Note 1: This doesn't run unless the IDE is connected and the framebuffer is enabled.
    // Note 2: In double buffering mode, compress the alternate buffer which was written to.
    image_t img;
    framebuffer_initialize_image(&img);
    // This doesn't have to be valid on the first frame compression is skipped.
    img.pixels = sensor->buffers[sensor->buf_idx ^ 1];
    framebuffer_update_jpeg_buffer(&img);
    ...
    // The first buffer always points to the start of the framebuffer.
    // The second buffer is set later if double buffering is enabled, and there's enough memory.
    sensor->buffers[0] = MAIN_FB()->pixels;
    ...
    switch (sensor->pixformat) {
        ...
        case PIXFORMAT_JPEG:
            ...
            // No double buffering for JPEG for now.
            flags = flags & (~SNAPSHOT_DBLBUF);
            break;
        default:
    ...
    if (snapshot_in_progress == false) { // cleared in dcmi_abort();
        // Start the snapshot.
        sensor_start_snapshot(sensor, addr, length, h);
    }

    // Wait for the frame data.
    for (uint32_t tick_start = HAL_GetTick(); (DCMI->CR & DCMI_CR_CAPTURE); ) {
        __WFI();
    ...
    if (image != NULL) {
        ...
        image->pixels = sensor->buffers[sensor->buf_idx];
    }

    // If double buffering is enabled, check if there's enough memory for two frames.
    if ((flags & SNAPSHOT_DBLBUF) && ((length * 2) < framebuffer_get_buffer_size())) {
        // If so, set the second framebuffer pointer.
        image_t img;
        framebuffer_initialize_image(&img);
        //Note we use the actual (possibly cropped) frame size to set the second buffer pointer.
        sensor->buffers[1] = MAIN_FB()->pixels + image_size(&img);
    } else {
        // If not, silently force single buffer mode.
        flags = flags & (~SNAPSHOT_DBLBUF);
        sensor->buf_idx    = 0;
        sensor->buffers[1] = MAIN_FB()->pixels;
    }

    if (flags & SNAPSHOT_DBLBUF) {
        // Swap the target framebuffer for the next snapshot.
        // Note: The next frame compression will use the other buffer.
        sensor->buf_idx ^= 1;

        // Start another snapshot before returning.
        sensor_start_snapshot(sensor, addr, length, h);
    }

    // Make sure the framebuffer is aware of double buffering state
    // to account for the doubled frame size when fb_alloc is used.
    framebuffer_set_doublebuf((flags & SNAPSHOT_DBLBUF) != 0);
    return 0;
}
