...
static int reset(sensor_t *sensor)
{
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        if (LEP_GetCameraBootStatus(&tmp_handle, &status) != LEP_OK) return -1;
    ...
    for (int i = 0; i < 1000; i++) {
        LEP_UINT16 status;
        if (LEP_DirectReadRegister(&tmp_handle, LEP_I2C_STATUS_REG, &status) != LEP_OK) return -1;
    ...
    for (int i = 0; i < 5000; i++) {
        ...
        if (LEP_GetSysFFCStatus(&tmp_handle, &status) != LEP_OK) return -1;
    ...
    if (LEP_SetRadEnableState(&tmp_handle, LEP_RAD_DISABLE) != LEP_OK) return -1;

    LEP_AGC_ROI_T roi;
    if (LEP_GetAgcROI(&tmp_handle, &roi) != LEP_OK) return -1;
    int tmp_h_res = roi.endCol + 1;
    int tmp_v_res = roi.endRow + 1;

    if (LEP_SetAgcEnableState(&tmp_handle, LEP_AGC_ENABLE) != LEP_OK) return -1;
    if (LEP_SetAgcCalcEnableState(&tmp_handle, LEP_AGC_ENABLE) != LEP_OK) return -1;
...
{
    uint16_t reg_data;
    if (cambus_readw2(sensor->slv_addr, reg_addr, &reg_data)) return -1;
    return reg_data;
}
...
const int vospi_line_pixels = 80;
const int vospi_number_packets = 60;
const int vospi_special_line = 20;

int alt_snapshot_src(sensor_t *sensor)
{
    if ((!h_res) || (!v_res) || (!sensor->framesize) || (!sensor->pixformat)) return -1;
...
int lepton_init(sensor_t *sensor)
{
    ...
    sensor->alt_snapshot_src    = alt_snapshot_src;
