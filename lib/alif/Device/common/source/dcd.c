...
// TinyUSB 0.21.0 added an is_isr argument to the dcd_edpt_xfer() APIs. The
// firmware builds against MicroPython's TinyUSB, the bootloader against ours,
// and those are not always the same version, so support both signatures.
#if TUSB_VERSION_NUMBER >= 2100
#define DCD_XFER_IS_ISR_ARG   , bool is_isr
#define DCD_XFER_IS_ISR       , false
#else
#define DCD_XFER_IS_ISR_ARG
#define DCD_XFER_IS_ISR
#endif
...
void dcd_set_address(uint8_t rhport, uint8_t dev_addr)
{
    ...
    dcd_edpt_xfer(rhport, tu_edpt_addr(0, TUSB_DIR_IN), NULL, 0 DCD_XFER_IS_ISR);
}
...
// notify the stack
bool dcd_edpt_xfer(uint8_t rhport, uint8_t ep_addr, uint8_t * buffer, uint16_t total_bytes DCD_XFER_IS_ISR_ARG)
{
    #if TUSB_VERSION_NUMBER >= 2100
    (void) is_isr;
    #endif
...
bool dcd_edpt_xfer_fifo(uint8_t rhport, uint8_t ep_addr, tu_fifo_t * ff, uint16_t total_bytes DCD_XFER_IS_ISR_ARG) TU_ATTR_WEAK;
