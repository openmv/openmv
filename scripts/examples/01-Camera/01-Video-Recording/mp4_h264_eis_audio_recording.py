...
#
# H.264 MP4 Recording with Electronic Image Stabilization + Audio
#
...
with mp4.Mp4("stabilized_audio.mp4", W, H, fps=FPS,
    ...
    while time.ticks_diff(time.ticks_ms(), start) < RECORD_TIME * 1000:
        ...
        ts = time.ticks_us()
        au = encoder.encode(img, roi=(x, y, W, H), timestamp_us=ts)
...
encoder.deinit()
print("done: stabilized_audio.mp4")
