import struct
class ModbusRTU():
    def __init__(self, uart, slave_id=0x01):
    ...
    def handle(self):
        ...
        if self.SLAVE_ID != additional_address:
            ...
            RESPONSE += struct.pack('b', 0x04)#SLAVE_ID
        elif self.crc16(REQUEST[:-2]) != error_check:
            RESPONSE += struct.pack('b', function_code|0x80)
            RESPONSE += struct.pack('b', 0x04)#CRC error
        elif function_code == 0x03:
            starting_address = struct.unpack('>h', data[:2])[0]
            quantity_of_registers = struct.unpack('>h', data[2:4])[0]
            response_registers = self.HOLDING_REGISTERS[starting_address:starting_address+quantity_of_registers]
            ...
            RESPONSE += struct.pack('>h', self.HOLDING_REGISTERS[register_address])
        elif function_code == 0x10:
            starting_address = struct.unpack('>h', data[:2])[0]
            quantity_of_registers = struct.unpack('>h', data[2:4])[0]
            byte_of_registers = struct.unpack('b', data[4:5])[0]
            values = struct.unpack('>%dh'%quantity_of_registers, data[5:])
            self.HOLDING_REGISTERS[starting_address:starting_address+quantity_of_registers] = values
            if byte_of_registers == 2*quantity_of_registers:
                ...
                RESPONSE += struct.pack('b', function_code|0x80)
                RESPONSE += struct.pack('b', 0x04)
        else:
            RESPONSE += struct.pack('b', function_code|0x80)
            RESPONSE += struct.pack('b', 0x01)#not support
