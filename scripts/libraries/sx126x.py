...
class SX126X:
    ...
    def readData(self, data, len_):
        ...
        payload_len, offset = self.getRxBufferStatus()
        length = payload_len if len_ == SX126X_MAX_PACKET_LENGTH else min(len_, payload_len) if len_ else payload_len
        state = self.readBuffer(data, length, offset)
        ASSERT(state)
    ...
    def getRxBufferStatus(self):
        """Returns (payload_length, rx_start_buffer_pointer) for correct FIFO read offset."""
        rxBufStatus = bytearray(2)
        rxBufStatus_mv = memoryview(rxBufStatus)
        self.SPIreadCommand([SX126X_CMD_GET_RX_BUFFER_STATUS], 1, rxBufStatus_mv, 2)
        return rxBufStatus[0], rxBufStatus[1]
    ...
    def readBuffer(self, data, numBytes, offset=0):
        cmd = [SX126X_CMD_READ_BUFFER, offset & 0xFF]
