...
class OMVCamera:
    ...
    def __init__(self, port, baudrate=921600, crc=True,
                 seq=True, ack=False, events=True,
    ...
    def _send_cmd_wait_resp(self, opcode, channel=0, data=b''):
        ...
        try:
            ...
            self._resync()
            # Raise resync exception to indicate that resync was performed
    ...
    def _handle_event(self, channel_id, event):
        ...
        if channel_id == 0:
            # System events
            try:
                event_name = OMVPEventType(event).name
            except ValueError:
                event_name = f"0x{event:04X}"
            logging.info(f"🔔 System Event: channel={channel_id}, event={event_name}")
            ...
            # Channel events
            channel_name = self.channels_by_id[channel_id]["name"]
            if event == 0xFFFF:
                logging.info(f"🔔 Channel Event: channel={channel_name}, event=NOTIFY")
            else:
                logging.info(f"🔔 Channel Event: channel={channel_name}, event=0x{event:04X}")
        else:
            # Unknown channel
    ...
    def _resync(self):
        ...
        # Reset transport to safe defaults and renegotiate capabilities
        self.transport.update_caps(crc=True, seq=True, ack=False, max_payload=OMVProto.MIN_PAYLOAD_SIZE)
        ...
        for attempt in range(self.max_retry):
            try:
                ...
                self.transport.send_packet(OMVPOpcode.PROTO_SYNC, 0, 0, b'')
        ...
        self.transport.update_caps(self.caps['crc'], self.caps['seq'], self.caps['ack'], self.caps['max_payload'])
    ...
    def connect(self):
        ...
        try:
            ...
            self.transport = OMVTransport(self._serial, crc=True, seq=True, ack=False,
                                          max_payload=OMVProto.MIN_PAYLOAD_SIZE,
                                          timeout=self.timeout, event_callback=self._handle_event,
    ...
    def exec(self, script):
        ...
        self._channel_write(stdin_id, script)
        # Execute the script
    ...
    @_retry_on_resync
    def channel_size(self, channel_name):
        """Get size of data available in a custom channel"""
        channel_id = self.get_channel(name=channel_name)
        return 0 if channel_id is None else self._channel_size(channel_id)
    
    @_retry_on_resync
    def channel_read(self, channel_name, size=None):
        """Read data from a custom channel"""
        if channel_id := self.get_channel(name=channel_name):
            if size is None:
    ...
    @_retry_on_resync
    def channel_write(self, channel_name, data):
    ...
    def has_channel(self, channel_name):
        ...
        return self.get_channel(name=channel_name) is not None
