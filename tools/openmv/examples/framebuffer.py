...
def main():
    ...
    try:
        with OMVCamera(args.port, crc=args.crc, seq=args.seq,
            ...
            camera.stop()
            camera.exec(script)
            camera.streaming(True, raw=True, res=(512, 512))
            ...
            while True:
                ...
                # Handle frame data
                if status and status.get('stream'):
                    frame = camera.read_frame()
                    if frame:
                        fps = fps_clock.get_fps()
                        w, h = frame['width'], frame['height']
                        data = frame['data']
                        fmt = frame.get('format_str', 'RGB888')
                        ...
                        # Draw FPS info
                        fps_text = f"{fps:.2f} FPS {fps * len(data) / 1024**2:.2f} MB/s {w}x{h} {fmt}"
