"""
OpenMV Protocol V2 Transport Layer
...
import logging
import struct
from .constants import OMVProto, OMVPState, OMVPFlags, OMVPStatus, status_name
...
class OMVTransport:
    ...
    def __init__(self, serial, crc_enabled, seq_enabled, ack_enabled, max_payload, timeout):
        self.serial = serial
        ...
        self.packet = Packet(crc_enabled, seq_enabled, max_payload)
    ...
    def update_caps(self, crc_enabled, seq_enabled, ack_enabled, max_payload):
    ...
    def recv_packet(self):
        ...
        while time.time() - start_time < self.timeout:
            ...
            if self.ack_enabled and not (packet['flags'] & (OMVPFlags.ACK | OMVPFlags.NAK)):
            ...
            if packet['flags'] & OMVPFlags.NAK:
                ...
                logging.debug(f"Command failed with status: {status_name(status)}")
                ...
                if status == OMVPStatus.CHECKSUM:
                    ...
                    raise OMVPException(f"Command failed with status: {status_name(status)}")
                return False
